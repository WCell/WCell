...
namespace NHibernate.Type
{
	...
	public abstract class NullableType : AbstractType
	{
		...
		/// <param name="rs"></param>
		/// <returns></returns>
		public virtual object NullSafeGet(IDataReader rs, int index)
		{
			if (rs.IsDBNull(index))
			{
				// TODO: add a method to NullableType.GetNullValue - if we want to
			...
			else
			{
				...
				catch (InvalidCastException ice)
				{
					throw new ADOException(
						string.Format(
							"Could not cast the value in column #{0} of type {1} to the Type {2}.  Please check to make sure that the mapping is correct and that your DataProvider supports this Data Type.",
							index, rs[index].GetType().Name, GetType().Name), ice);
				}
