	...
	public class Table : IRelationalModel
	{
		...
		public string SqlCreateString(Dialect.Dialect dialect, IMapping p, string defaultCatalog, string defaultSchema)
		{
			try
			{
				...
				if (!string.IsNullOrEmpty(Settings.DefaultCharset))
				{
					buf.Append(" " + dialect.GetTableCharsetString(Settings.DefaultCharset));
				}
			...
			{
				throw new Exception("Unable to create SQL-string for Table: " + name, e);
			}
		}
