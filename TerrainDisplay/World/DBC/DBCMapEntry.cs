...
namespace TerrainDisplay.World.DBC
{
    public class DBCMapEntryConverter : AdvancedDBCRecordConverter<MapInfo>
    {
        public override MapInfo ConvertTo(byte[] rawData, ref int id)
        {
            ...
            info.Id = (MapId) (id = GetInt32(rawData, i++));
            info.InternalName = GetString(rawData, TerrainDisplayConfig.DefaultLocale, i++);
            info.MapType = (MapType) GetInt32(rawData, i++);
            info.Unknown1 = GetUInt32(rawData, i++);
            info.HasTwoSides = (GetUInt32(rawData, i++) == 1);
            info.Name = GetString(rawData, TerrainDisplayConfig.DefaultLocale, i++);

            i = 21;
            info.UnknownFlags1 = GetUInt32(rawData, i++);
            info.AreaTable = GetInt32(rawData, i++);
            info.AllianceText = GetString(rawData, TerrainDisplayConfig.DefaultLocale, i++);

            i = 39;
            info.UnknownFlags2 = GetUInt32(rawData, i++);
            info.HordeText = GetString(rawData, TerrainDisplayConfig.DefaultLocale, i++);
