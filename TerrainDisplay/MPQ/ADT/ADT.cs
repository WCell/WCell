...
namespace MPQNav.MPQ.ADT
{
    public class ADT
    {
        ...
        public ADT(string filePath, TerrainManager terrainManager)
        {
        ...
        {
            if (Vertices == null) return Vector3.Zero;
            if (Vertices.Count == 0) return Vector3.Zero;

            var minVec = new Vector3(float.MaxValue, float.MaxValue, float.MaxValue);
            foreach (var vertex in Vertices)
            {
                if (vertex.Position.X < minVec.X)
                {
