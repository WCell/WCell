...
namespace WCell.Addons.Default.Instances
{
    ...
    public class PrinceKelesethAttackAction : AIAttackAction
    {
        ...
        private NPCEntry[] vrykulSkeletonArrayEntry = new NPCEntry[4];
        private NPC[] SkeletonsMinions = new NPC[4];
        
        private Vector3[] vrykulSkeletonVector = new Vector3[5]
        { 
        ...
        public override void Start()
        {
            ...
            // Summon 4 Vrykul Skeleton
            for (int i = 0; i < 4; i++)
            {

                vrykulSkeletonArrayEntry[i] = NPCMgr.GetEntry(NPCId.VrykulSkeleton);

                vrykulSkeletonArrayEntry[i].AddSpell(SpellId.Decrepify);
                SpellHandler.Apply(spell => { spell.CooldownTime = 5000; }, SpellId.Decrepify);

                
                //vrykulSkeletonArrayEntry[i].Activated += vrykulSkeleton =>
                //{
                //    ((BaseBrain)vrykulSkeleton.Brain).DefaultCombatAction.Strategy = new VrykulSkeletonAttackAction(vrykulSkeleton);

                //};
                
                SkeletonsMinions[i] = m_owner.SpawnMinion(vrykulSkeletonArrayEntry[i], ref vrykulSkeletonVector[i], 0);
                //SkeletonsMinions[i].Target = m_owner.GetNearbyRandomHostileCharacter();
                //SkeletonsMinions[i].Movement.MoveTo(m_owner.GetNearbyRandomHostileCharacter().Position);
                //SkeletonsMinions[i].Health = 0;
        ...
        {
            int countSkeletonsDead = 0;

            for(int i = 0; i < 4; i++){

                if (!SkeletonsMinions[i].IsAlive) {
                    countSkeletonsDead++;
                };
            }

            if (countSkeletonsDead == 4)
            {

                m_owner.SpellCast.Start(SpellId.SoulstoneResurrection_7, false, SkeletonsMinions);
