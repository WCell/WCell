...
namespace WCell.RealmServer.Global
{
	...
	public partial class Region : IGenericChatTarget, IRegionId, IContextHandler, IWorldSpace
	{
		...
		//    // get the time at the start of our callback
		//    long updateStart = m_regionTime.ElapsedMilliseconds;
		...
		//    // get the current region time
		//    var now = m_regionTime.ElapsedMilliseconds;

		//    // check to see if it's time to run a region update yet
		//    if (m_lastRegionUpdate + WCellDef.REGION_UPDATE_INTERVAL <= now)
		//    {
		//        // Timers
		//        var timerStart = m_regionTime.ElapsedMilliseconds;

		//        // see if we have any messages to execute
		//        if (m_timerList.Count > 0)
		//        {
		//            // update delta in seconds
		//            var updateDt = ((timerStart - m_lastTimerUpdate) / 1000.0f);

		//            foreach (var updatable in m_timerList)
		//            {
		//                updatable.Update(updateDt);
		//            }
		//        }

		//        m_lastTimerUpdate = timerStart;
		//        // Updates
		//        //HashSet<WorldObject> objUpdateQueue = null;
		//        //Utility.SwapReference(ref m_objectsToUpdate, ref objUpdateQueue, new HashSet<WorldObject>());

		//        //foreach (var obj in objUpdateQueue)
		//        //{
		//        //    if (obj.Region == this)
		//        //    {
		//        //        // make sure, Object was not removed from Region
		//        //        obj.BroadcastValueUpdate();
		//        //        obj.ResetUpdateInfo();
		//        //    }
		//        //}

		//        if (m_lastCharacterUpdate + WCellDef.CHARACTER_WORLD_UPDATE_INTERVAL <= now)
		//        {
		...
		//            m_lastCharacterUpdate = now;
		//        }

		//        // update delta in seconds
		//        float objUpdateDelta = ((now - m_lastRegionUpdate) / 1000.0f);

		//        // notify each region object
		//        foreach (var obj in m_objects.Values)
		//        {
		//            obj.Update(objUpdateDelta);
		//        }
		...
		//        // we updated the region, so set our last update time to now
		//        m_lastRegionUpdate = now;
		//    }

		//    // get the time, now that we've finished our update callback
		//    long updateEnd = m_regionTime.ElapsedMilliseconds;

		//    if (m_running)
		//    {
		//        long callbackTimeout = (updateStart + m_updateDelay) - updateEnd;
		//        if (callbackTimeout < 0)
		//        {
		//            // even if we are in a hurry: For the sake of load-balance we have to give control back to the ThreadPool
		//            callbackTimeout = 0;
