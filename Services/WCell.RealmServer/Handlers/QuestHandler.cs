...
namespace WCell.RealmServer.Handlers
{
	...
	public static class QuestHandler
	{
        ...
        [ClientPacketHandler(RealmServerOpCode.CMSG_START_QUEST)]
        public static void HandleStartQuest(IRealmClient client, RealmPacketIn packet)
        {
...
+    SendPacket(&data);
+}*/
		[ClientPacketHandler(RealmServerOpCode.SMSG_QUERY_QUESTS_COMPLETED_RESPONSE)]
		...
		public static void SendQuestQueryResponse(QuestTemplate qt, Character chr)
		{
			...
			using (var pckt = new RealmPacketOut(RealmServerOpCode.SMSG_QUEST_QUERY_RESPONSE))
			{
			    ...
			    pckt.Write(qt.Flags.HasFlag(QuestFlags.HiddenRewards) ? 0 : qt.RewMoney);
				...
				pckt.Write(0);										// since 3.3: reward skill line id
                pckt.Write(0);										// reward skill points
                pckt.Write(0);										// review reputation show mask
                pckt.Write(0);										// questgiver portrait ID
                pckt.Write(0);										// quest turn in portrait ID
			    ...
			    pckt.Write(0u); //required spell

				for (i = 0; i < QuestConstants.MaxObjectiveTexts; i++)
				{
				    var set = qt.ObjectiveTexts[(int)locale];
				    pckt.Write(set != null ? set.Texts[i] : "");
				}

			    for (i = 0; i < 4; i++)
                {
                    pckt.Write(0u); //reward currency
                    pckt.Write(0u); //count
                }

                for (i = 0; i < 4; i++)
                {
                    pckt.Write(0u); //required currency
                    pckt.Write(0u); //count
                }
		...
		/// </summary>
		/// <param name="questGiver">The entity that is sending the quest details.</param>
		/// <param name="qt">The quest template that is being described.</param>
		/// <param name="chr">The client to recieve the details.</param>
		...
		public static void SendDetails(IEntity questGiver, QuestTemplate qt, Character chr, bool acceptable)
		{
			...
			using (var pckt = new RealmPacketOut(RealmServerOpCode.SMSG_QUESTGIVER_QUEST_DETAILS))
			{
                ...
                /// If this is not zero then it will cause the client
                /// to send a packet back (8868) decimal in 406
                /// with the players guid, quest sharing?
				...
				{
                    for (i = 0; i < 6; i++ )
                        pckt.WriteUInt(0u); //item ids
                    for (i = 0; i < 6; i++)
                        pckt.WriteUInt(0u); //item amounts
                    for (i = 0; i < 6; i++)
                        pckt.WriteUInt(0u); //item display ids
				}
				else
				{
					for (i = 0; i < qt.RewardChoiceItems.Length &&  i < 6; i++)
					{
					    pckt.Write((uint) qt.RewardChoiceItems[i].ItemId);
					}
                    for (; i < 6; i++ )
                ...
                for (i = 0; i < qt.RewardItems.Length &&  i < 4; i++)
                {
                    pckt.Write((uint) qt.RewardItems[i].ItemId);
                }
			    for (; i < 4; i++ )
				...
				for (i = 0; i < QuestConstants.MaxEmotes; i++)
				{
		...
		public static void SendQuestGiverOfferReward(IEntity questGiver, QuestTemplate qt, Character chr)
		{
			...
			using (var pckt = new RealmPacketOut(RealmServerOpCode.SMSG_QUESTGIVER_OFFER_REWARD))
			{
                ...
                var i = 0;
				for (; i < qt.OfferRewardEmotes.Length; i++)
				{
