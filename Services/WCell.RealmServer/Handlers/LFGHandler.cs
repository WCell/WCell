...
namespace WCell.RealmServer.Handlers
{
	...
	public static class LFGHandler
	{
		...
		[ClientPacketHandler(RealmServerOpCode.CMSG_SEARCH_LFG_JOIN)]
		public static void HandleSearchJoin(IRealmClient client, RealmPacketIn packet)
		{
			var dungeonId = packet.ReadUInt32();
		}

		[ClientPacketHandler(RealmServerOpCode.CMSG_SEARCH_LFG_LEAVE)]
		public static void HandleSearchLeave(IRealmClient client, RealmPacketIn packet)
		{

		}

		[ClientPacketHandler(RealmServerOpCode.CMSG_LFG_PROPOSAL_RESULT)]
		...
		[ClientPacketHandler(RealmServerOpCode.CMSG_LFG_SET_BOOT_VOTE)]
		public static void HandleBootPlayerVote(IRealmClient client, RealmPacketIn packet)
		{
			var passVote = packet.ReadBoolean();
		}

		[ClientPacketHandler(RealmServerOpCode.CMSG_LFD_PLAYER_LOCK_INFO_REQUEST)]
		...
		[ClientPacketHandler(RealmServerOpCode.CMSG_LFD_PARTY_LOCK_INFO_REQUEST)]
		...
		public static void SendUpdateSearch(IRealmClient client)
		{
			...
			using (var packet = new RealmPacketOut(RealmServerOpCode.SMSG_LFG_UPDATE_LIST))
			{
				packet.Write(true);
				client.Send(packet);
			}
