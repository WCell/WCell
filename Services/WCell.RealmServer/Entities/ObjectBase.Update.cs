...
namespace WCell.RealmServer.Entities
{
	public partial class ObjectBase
	{
		...
		{
			return (_UpdateFieldInfos.FieldFlags[fieldIndex] & (UpdateFieldFlags.Public | UpdateFieldFlags.Dynamic)) != 0;
		}
		...
		protected internal virtual void WriteObjectCreationUpdate(Character chr)
		{
            ...
            if (updateFlags.Has(UpdateFlags.Living | UpdateFlags.StationaryObject | UpdateFlags.StationaryObjectOnTransport))
            {
                WriteMovementUpdate(packet, relation);
            }
		    if (updateFlags.Has(UpdateFlags.Flag_0x8))
            {
                WriteUpdateFlag_0x8(packet, relation);
            }
            if (updateFlags.Has(UpdateFlags.Flag_0x10))
            {
		...
		protected void WriteUpdateValues(bool forCreation, Character receiver, UpdateFieldFlags relation)
		{
			...
			if (forCreation)
			{
				...
				for (var i = 0; i <= m_highestUsedUpdateIndex; i++)
				{
					...
					if ((flags & relation) != 0 && m_updateValues[i].UInt32 != 0)
					{
						mask.SetBit(i);
		    ...
		    if ((relation & UpdateFieldFlags.Private) != 0)
		    {
		        // Private
		        mask = m_privateUpdateMask;
		    }
		    else if ((relation & (UpdateFieldFlags.OwnerOnly | UpdateFieldFlags.GroupOnly)) != 0)
		    {
		        // Group or Owner
		        var pos = receiver.m_updatePacket.Position;
		        ...
		        for (var i = m_privateUpdateMask.m_lowestIndex; i <= m_privateUpdateMask.m_highestIndex; i++)
		        {
		            var flags = _UpdateFieldInfos.FieldFlags[i];
		            if ((flags & relation) != 0 && 
						((flags & UpdateFieldFlags.Public) == 0 || m_publicUpdateMask.GetBit(i)))
		            {
		                mask.SetBit(i);
		                WriteUpdateValue(receiver.m_updatePacket, receiver, i);
		            }
