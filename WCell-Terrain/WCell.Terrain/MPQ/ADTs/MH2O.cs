				...
				if (heights == null)
				{
					...
					for (var x = 0; x <= TerrainConstants.UnitsPerChunkSide; x++)
					{
						for (var y = 0; y <= TerrainConstants.UnitsPerChunkSide; y++)
						{
							...
							//if (((x < Header.XOffset) || (x > (Header.XOffset + Header.Height))) ||
							//								((y < Header.YOffset) || (y > (Header.YOffset + Header.Width))))
							if (((x < Header.XOffset) || (x > (Header.XOffset + Header.Width))) ||
								((y < Header.YOffset) || (y > (Header.YOffset + Header.Height))))
							{
								continue;
							}

							heights[x, y] = HeightsArray[x + y * x];
						}
					}
				}
				return heights;
