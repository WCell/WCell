...
namespace WCell.Terrain.Serialization
{
    ...
    public static class ADTWriter
	{
		...
		public const string FileTypeId = "ter";
	    ...
	    private static void ClearObjectData()
	    {
            ...
            WorldObjectWriter.LoadedM2Models.Clear();
	        WorldObjectWriter.LoadedWMORoots.Clear();
	    }
        ...
        {
			var adt = new ADT(new Point2D(0, 0), wdt);
		...
		public static ADT[,] ExtractMapTiles(WDT wdt)
		{
			...
			for (var x = 0; x < TerrainConstants.TilesPerMapSide; x++)
			{
				for (var y = 0; y < TerrainConstants.TilesPerMapSide; y++)
				{
					...
					var adt = ADTReader.ReadADT(WDTWriter.MpqFinder, wdt, new Point2D(x, y));
                    if (adt == null) continue;
				    ...
				    PrepareChunkInfo(wdt.Finder, adt);
	    ...
	    private static void PrepareChunkInfo(MPQFinder finder, ADT adt)
	    {
            for (var chunkX = 0; chunkX < TerrainConstants.ChunksPerTileSide; chunkX++)
            {
                ...
                {
                    var chunk = (ADTChunk)adt.Chunks[chunkY, chunkX];
                    foreach (var dRefId in chunk.DoodadRefs)
                    {
                        ...
                        M2Model model;
                        if (!WorldObjectWriter.LoadedM2Models.TryGetValue(dRef.FilePath, out model))
                        {
                            model = Serialization.M2Reader.ReadM2(finder, dRef.FilePath);
                            WorldObjectWriter.LoadedM2Models.Add(dRef.FilePath, model);
                        }
                        ...
                        if (model.BoundingVertices.IsNullOrEmpty())
                        {
                            ...
                            WorldObjectWriter.LoadedM2Models.Remove(dRef.FilePath);
                        }
        ...
        private static void PrepareWMO(MPQFinder finder, MapObjectDefinition def)
	    {
	    ...
	    {
			//var chunk = adt.Chunks[chunkY, chunkX];
			...
			//if (chunk.IsFlat)
			//    Console.WriteLine("Found flat chunk: {0},{1}.", chunk.X, chunk.Y);
		...
		public static void WriteTileInfo(FileStream file, ADT adt)
		{
			using (var writer = new BinaryWriter(file))
			{
				...
				for (var x = 0; x < TerrainConstants.ChunksPerTileSide; x++)
				{
					...
					{
						var chunk = (ADTChunk)adt.Chunks[y, x];
        ...
        private static void WriteChunkInfo(BinaryWriter writer, TerrainChunk chunk)
	    {
            writer.Write(chunk.NodeId);
	        writer.Write(chunk.IsFlat);
	        
			// The base height for this chunk
	        writer.Write(chunk.MedianHeight);

	        // TODO: The wmos and m2s (UniqueIds) that overlap this chunk
			//WriteChunkModelRefs(writer, chunk.DoodadRefs);
			//WriteChunkObjRefs(writer, chunk.ObjectRefs);

			writer.Write(chunk.HolesMask);

			// The height map
			if (!chunk.IsFlat)
			{
				WriteChunkArray(writer, chunk.Heights.GetLowResMapMatrix());
			}

			// The liquid information);
			writer.Write(chunk.IsLiquid);
            if (!chunk.IsLiquid)
            {
                return;
            }
	        ...
	        WriteChunkArray(writer, chunk.LiquidHeights);
	    }
