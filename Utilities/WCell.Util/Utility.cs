...
namespace WCell.Util
{
	...
	public static class Utility
	{
		...
		public static readonly Dictionary<string, Type> TypeMap = new Dictionary<string, Type>(StringComparer.InvariantCultureIgnoreCase);
					...
					if (!type.FullName.StartsWith("System.") && !type.FullName.StartsWith("Microsoft."))
					{
						if (type.IsValueType)
						{
							...
							if (type.IsEnum)
							{
								...
								{
									var dict = new Dictionary<string, object>(values.Length + 100, StringComparer.InvariantCultureIgnoreCase);
		...
		public static Dictionary<Type, Func<string, object>> TypeParsers = new Func<Dictionary<Type, Func<string, object>>>(() =>
		{
			var parsers = new Dictionary<Type, Func<string, object>>();

			parsers.Add(typeof(int), strVal => int.Parse(strVal));

			parsers.Add(typeof(float), strVal => float.Parse(strVal));

			parsers.Add(typeof(long), strVal => long.Parse(strVal));

			parsers.Add(typeof(ulong), strVal => ulong.Parse(strVal));

			parsers.Add(typeof(bool), strVal => strVal.Equals("true", StringComparison.InvariantCultureIgnoreCase) ||
												strVal.Equals("1", StringComparison.InvariantCultureIgnoreCase) ||
												strVal.Equals("yes", StringComparison.InvariantCultureIgnoreCase));

			parsers.Add(typeof(double), strVal => double.Parse(strVal));

			parsers.Add(typeof(uint), strVal => uint.Parse(strVal));

			parsers.Add(typeof(short), strVal => short.Parse(strVal));

			parsers.Add(typeof(ushort), strVal => short.Parse(strVal));

			parsers.Add(typeof(byte), strVal => byte.Parse(strVal));
		...
		public static object Parse(string stringVal, Type type)
		{
			...
			{
				throw new Exception(string.Format("Unable to parse string-Value \"{0}\" as Type \"{1}\"", stringVal, type));
			}
			return obj;
		}
		...
		{
			return (type.Attributes & (TypeAttributes.Abstract | TypeAttributes.Sealed)) == (TypeAttributes.Abstract | TypeAttributes.Sealed);
		}
