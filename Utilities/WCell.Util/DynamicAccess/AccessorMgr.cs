...
namespace WCell.Util.DynamicAccess
{
	public static class AccessorMgr
	{
		...
		private static readonly Dictionary<Type, IProducer> m_defaultCtors =
			new Dictionary<Type, IProducer>();
		...
		public static IProducer GetOrCreateDefaultProducer(Type type)
		{
			...
			producer = CreateDefaultCtor(type);
			return producer;
		}
		...
		{
			var module = GetOrCreateModule();
			var producer = AddToModule(module, type.GetConstructor(new Type[0]));

			m_defaultCtors.Add(type, producer);
		...
		internal static IProducer AddToModule(ModuleBuilder module, ConstructorInfo ctor)
		{
			...
			{
				throw new ArgumentException("Cannot emit Constructors of value-types (yet): " + ctor.DeclaringType);
			}
			...
			// Emit
			var method = getMethod.GetILGenerator();
