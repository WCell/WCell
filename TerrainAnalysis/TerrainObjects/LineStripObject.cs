...
namespace TerrainAnalysis
{
    ...
    public class LineStripObject
    {
        ...
        public Microsoft.DirectX.Vector3 ObjectDirection
        {
            get
            {
                return this.pointer;
        ...
        public void CorrectObjectPosition(Microsoft.DirectX.Vector3 correctionVector, float dampeningFactor)
        {
            float correctionMagnitude = dampeningFactor * correctionVector.Length();
            correctionVector.Normalize();

            float angle = (float)Math.Acos(Microsoft.DirectX.Vector3.Dot(correctionVector,this.normal));
            if (angle > Math.PI * 0.5f)
                this.position -= this.normal * correctionMagnitude;
            else
                this.position += this.normal * correctionMagnitude;
        }
    }
