...
using WCell.Util.Toolshed;using System.IO;
...
namespace WCell.Core.Commands
{
		...
		where C : ICmdArgs
	{
		...
		/// </summary>
		readonly internal IDictionary<int, BaseCommand<C>> allCommandsByType = new Dictionary<int, BaseCommand<C>>();
		//public readonly HelpCommand HelpCommandInstance;

	    protected CommandMgr()
		{
		...
		public bool ExecuteAll(CmdTrigger<C> trigger, List<Command<C>> commands)
		{
			foreach (Command<C> cmd in commands)
			{
		...
		public BaseCommand<C> GetCommand(CmdTrigger<C> trigger)
		{
			string alias;
		...
		public virtual bool Trigger(CmdTrigger<C> trigger, BaseCommand<C> cmd)
		{
			if (cmd.Enabled)
			{
				var rootCmd = cmd.RootCmd;
				if (!rootCmd.MayTrigger(trigger, cmd, false))
				{
					//trigger.Reply("Cannot trigger Command: " + rootCmd);
				}
				else if (TriggerValidator(trigger, cmd, false))
				{
					trigger.cmd = cmd;
		...
		public void Add(Command<C> cmd)
		{
			...
			var ptr = cmd.GetType().TypeHandle.Value.ToInt32();
			allCommandsByType.Add(ptr, cmd);

			cmd.mgr = this;
			foreach (var subCmd in cmd.SubCommands)
			{
				ptr = subCmd.GetType().TypeHandle.Value.ToInt32();
		...
		/// <summary>
		/// Adds a command of the specific type
		/// </summary>
		...
		public void Remove(Command<C> cmd)
		{
			...
			allCommandsByType.Remove(cmd.GetType().TypeHandle.Value.ToInt32());
			foreach (var subCmd in cmd.SubCommands)
			{
				allCommandsByType.Remove(subCmd.GetType().TypeHandle.Value.ToInt32());
			}
		}
		...
		{
			BaseCommand<C> cmd;
			allCommandsByType.TryGetValue(typeof(T).TypeHandle.Value.ToInt32(), out cmd);
			return (T)cmd;
		}
		...
		public bool IsAllowedInContext(CmdTrigger<C> trigger, BaseCommand<C> cmd, bool ignoreRestrictions)
		{
			return cmd.Enabled &&
				   (ignoreRestrictions || (cmd.RootCmd.MayTrigger(trigger, cmd, true) && TriggerValidator(trigger, cmd, true)));
		}
		...
		public void TriggerHelp(CmdTrigger<C> trigger, bool ignoreRestrictions)
		{
			if (trigger.Text.HasNext)
			{
				...
				if (cmds.Count > 0)
				{
					foreach (var cmd in cmds)
					{
						if (IsAllowedInContext(trigger, cmd, ignoreRestrictions))
						{
		...
		{
			trigger.Reply(string.Format("{0}{1}", cmd.Usage, detail ? "(" + cmd.Description + ")" : ""));
			...
			if (cmd.SubCommands.Count > 0)
			{
				...
				foreach (var subCmd in cmd.SubCommands)
				{
					if (IsAllowedInContext(trigger, subCmd, ignoreRestrictions))
					{
		...
		public class CallCommand : Command<C>
		{
			...
			{
				Init("Call", "C", "@", "$");
				ParamInfo = "(-l|-<i>)|<methodname>[ <arg0> [<arg1> [...]]]";
				Description = "Calls any static method or custom function with the given arguments. Either use the name or the index of the function.";
			}
			...
			public override void Process(CmdTrigger<C> trigger)
			{
				...
				else
				{
					...
					{
						var args = new object[exec.ParameterTypes.Length];
						object value = null;
						for (var i = 0; i < exec.ParameterTypes.Length; i++)
						{
							var paramType = exec.ParameterTypes[i];
							Utility.Parse(txt.NextWord(), paramType, ref value);
							args[i] = value;
						}
						exec.Exec(args);
					}
